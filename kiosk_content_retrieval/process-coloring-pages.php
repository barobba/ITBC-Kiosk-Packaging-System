...
function process_coloring_pages($data_source_url, $results_directory) {
  ...
  verbose("Processing coloring pages\n");
  ...
  foreach ($nids as $nid) {
    ...
    foreach ($book->pictures as $picture) {
      ...
      if (!empty($filename)) {
        ...
        verbose(print_r('Could not parse this URL: '.$pictureURL, TRUE));
      }
  ...
  verbose("Finished processing coloring pages\n");
